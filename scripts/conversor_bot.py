...
def Return_Equivalent_Currency(cambio='', amount=''):
    ...
    try:
        ...
        driver.quit()
        print(f'{cambio.title()} hoje está à: R${moeda}')
        #Formatar a moeda para um formato float válido
        moeda = moeda.replace(",", ".")
        f_moeda = float(moeda)
        #print(f_moeda)      

        if input('Converter valor especifico? S/N').lower() == 's':
            amount = float(input(f'Quantos Reais para {cambio}?'))
            print(f'{amount} em {cambio} dá {amount * f_moeda}') 
